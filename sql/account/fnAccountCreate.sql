...
CREATE OR REPLACE FUNCTION account_create( "in_nickname" character varying, "in_email" character varying,
                                           "in_password" character varying, "sha_hash" character varying, "invite_key" character varying,
    ...
    BEGIN
        ...
        IF COALESCE("out_account_id", 0) <= 0 THEN
            INSERT INTO "Account" ("login", "password", "display_name",
                                   "last_name", "first_name", "print_name", "last_alt", "first_alt", "print_alt", "email",
            ...
            SELECT LOWER(TRIM(LEFT("in_nickname", 64))) as "login", encode(sha512(CAST(CONCAT("sha_hash", "in_password") AS bytea)), 'hex') as "password",
                   ...
                   TRIM(LEFT("in_lastname", 120)) as "last_name", TRIM(LEFT("in_firstname", 120)) as "first_name", TRIM(LEFT("in_printname", 256)) as "print_name",
                   TRIM(LEFT("in_lastalt", 120)) as "last_alt", TRIM(LEFT("in_firstalt", 120)) as "first_alt", TRIM(LEFT("in_printalt", 256)) as "print_alt",
                   ...
                   (SELECT z."code" FROM "Type" z WHERE z."is_deleted" = false and z."code" LIKE 'account.%' and z."code" = LOWER("in_type") LIMIT 1) as "type"
